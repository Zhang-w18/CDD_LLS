...
### 7.4 Reconstruction-C：local-window deterministic CDD-aware estimator

这也可称为 **coherence-band blockwise physical-channel reconstruction**。它是算法二的一个局部结构化分支，用来避免 Reconstruction-B 把全带宽所有 DMRS 拼成一个大 $\mathbf\Phi$ 后产生严重病态。其基本假设是：底层 physical branch channel 的相干带宽明显大于 CDD equivalent channel 的相干带宽。因此在一个不超过底层相干带宽的局部频域窗口内，可近似认为每个 Tx/effective port 的底层信道为常数。

该方法的关键是窗口宽度 $W$ 必须同时满足两个相反要求：

1. **底层信道局部平坦**：窗口不能太宽，否则 $H_{r,m}[k]$ 在窗口内不再近似常数；
2. **CDD 相位可辨识**：窗口不能太窄，否则不同 CDD-delayed branch 的 deterministic phase 差异不足，局部解耦矩阵条件数会很差。

因此理想工作区间是

$$
W \lesssim B_{c,H}(\gamma)
$$

同时窗口内 pilot 子载波集合 $\mathcal P_b$ 应产生足够大的 CDD 相位跨度。对 2Tx、delay vector $[0,d]$，若窗口内两个代表性 pilot 的间隔为 $\Delta k_{\mathrm{eff}}$，则相位差约为

$$
\Delta\phi_{\mathrm{CDD}}
=\frac{2\pi d\Delta k_{\mathrm{eff}}}{N_{\mathrm{FFT}}}.
$$

当 $\Delta\phi_{\mathrm{CDD}}$ 太小，两个端口的列向量接近共线，$\kappa(\mathbf A_b)$ 增大；当 $\Delta\phi_{\mathrm{CDD}}$ 接近 $\pi$ 附近时，2Tx 局部矩阵通常更容易解耦。换句话说，窗口存在性的直觉条件是

$$
S_f \le W \lesssim B_{c,H}(\gamma),
\quad
\frac{2\pi d W}{N_{\mathrm{FFT}}}
\text{ 足够大},
$$

其中 $S_f$ 是 DMRS 频域采样间隔。若底层物理信道相干带宽很大，而 CDD delay $d$ 也足够大，则可同时满足“底层近似常数”和“CDD 相位变化足够大”，此时该 local-window estimator 可能优于 Direct equivalent-channel RMMSE。
...
### 7.6 从 CDD 推广到任意 deterministic frequency-domain precoder

算法二并不只能用于 CDD。CDD 可以看作一种特殊的 deterministic frequency-domain precoder，其第 $m$ 个端口的频域权重为线性相位：
...
更一般地，若发端使用一个不依赖瞬时信道、但 UE 已知的频域预编码向量

$$
\mathbf c[k]
=
\left[c_0[k],c_1[k],\ldots,c_{N_t-1}[k]\right]^T,
\quad
\|\mathbf c[k]\|_2^2=1,
$$
...
c_m[k]H_{r,m}[k]
=
\mathbf c^T[k]\mathbf H_r[k].
$$

在 local-window reconstruction 中，若窗口 $\mathcal K_b$ 内底层信道近似常数：
...
因此，算法二的稳定性直接由窗口内 pilot sensing matrix $\mathbf A_b$ 的条件数决定。CDD 的问题是：$\mathbf c[k]$ 的相位由 delay $d_m$ 固定决定，可能在某些 DMRS spacing / window 下出现相位变化不足或 pilot 相位 alias，使 $\mathbf A_b$ 病态。若允许设计任意 deterministic frequency-domain precoder，可以把问题转化为：
...
\|\mathbf c[k]\|_2^2=1,
\quad
\mathbf c[k]\text{ 不依赖瞬时信道},
\quad
\mathbf c[k]\text{ 在频域提供足够分集。}
$$
...
$$
\mathbf A_b^H\mathbf A_b
=
\frac12
\begin{bmatrix}
P_b & \sum_{p\in\mathcal P_b}z[p]\\
\sum_{p\in\mathcal P_b}z^*[p] & P_b
\end{bmatrix},
$$

其中 $P_b=|\mathcal P_b|$。因此局部解耦稳定性的关键是让窗口内 pilot 上的相对相位 $z[p]$ 尽量均衡：

$$
\left|
\sum_{p\in\mathcal P_b}z[p]
\right|
\ll P_b.
$$

例如 QPSK cycling、Hadamard-like alternating phase、PRG-level QPSK cycling、或根据 DMRS pattern 离线优化的 pseudo-random QPSK sequence，都可以作为 CDD 之外的候选频域分集预编码。该类设计不使用瞬时 CSI，因此不是 beamforming；它是为了同时提供频域分集和改善算法二信道估计可辨识性的 deterministic precoder design。

需要注意：若某个设计在窗口内保持同一个 precoder，例如 PRG block 太宽，则 $\mathbf A_b$ 可能 rank deficient，无法恢复每个 branch channel；但它仍可能给出较好的 equivalent-channel NMSE。若目标是 physical branch reconstruction，应额外约束 $\kappa(\mathbf A_b)$ 和 branch NMSE；若目标只是数据等效信道估计，则 equivalent-channel NMSE 是主指标。

### 7.7 一般性频域预编码下的算法一 Direct RMMSE

一般性 deterministic frequency-domain precoder 下，算法一仍然可以使用。关键是：CDD 情况下的等效信道 covariance 可以写成 CDD-shifted PDP，因此只依赖 $\Delta k=k-k'$；但任意频域预编码下，等效信道 covariance 通常不再是 Toeplitz / WSS 形式，而是依赖具体的 $k,k'$ 位置。

设底层 branch channel covariance 为
...
若不同 Tx branch 独立、同分布，且共享同一 PDP，则
...
=
\sum_\ell p[\ell]
e^{-j2\pi\Delta k\ell/N_{\mathrm{FFT}}}.
$$
...
=
\sum_{m,n}
c_m[k]c_n^*[k']R_{mn}(k,k').
$$

在独立同分布 branch 假设下可简化为
...
只是此时 $\mathbf R_{g,\mathcal D\mathcal P}$ 和 $\mathbf R_{g,\mathcal P\mathcal P}$ 需要按 full matrix 逐元素构造，不能再只用一个 shifted PDP 生成 Toeplitz covariance。对 48 RB / 576 sc、$S_f=6$ 的场景，pilot 数 $N_P=96$，full covariance 的维度仍然可接受；复杂度主要是每个 precoder / SNR 配置预计算一次

$$
O(N_P^3)
$$

的矩阵求逆，以及每个 trial/Rx 的矩阵乘法。

结论：

1. 一般性频域预编码下，算法一不是不能用；
2. covariance 也不是不可求，只要 UE 知道 deterministic precoder pattern 和底层 branch PDP/covariance；
3. 但 covariance 不再能用 CDD-shifted PDP 的简单 stationary 表达，需要构造 full non-stationary covariance matrix；
4. 因此，若比较“优化 precoder + 算法二”是否真正优于最佳 direct LMMSE，应进一步加入 “优化 precoder + full-covariance 算法一” 作为更强 baseline。

从算法一角度也可以设计 deterministic precoder。其目标不再是改善 reconstruction matrix $\mathbf A_b$，而是让等效信道 covariance 在给定 DMRS pattern 下更容易由 pilot 预测 target RE。例如可用理论 LMMSE MSE：

$$
\mathrm{MSE}_{\mathrm{LMMSE}}
=
\mathrm{tr}(\mathbf R_{g,\mathcal D\mathcal D})
-
\mathrm{tr}\left[
\mathbf R_{g,\mathcal D\mathcal P}
\left(
...
+\sigma_{\mathrm{LS}}^2\mathbf I
\right)^{-1}
\mathbf R_{g,\mathcal P\mathcal D}
\right]
$$

作为离线设计目标。需要注意，若只优化 CE NMSE，最优解可能退化为 $z[k]$ 常数或低变化 precoder，因为这样等效信道最平滑、最容易估计，但它不一定提供频域分集。因此实际设计应同时约束：

1. CE NMSE / LMMSE MSE；
2. 频域 precoder 状态覆盖度或 diversity；
3. precoder transition granularity，例如按 4/8/12 RB cycling，而不是每个 RE 随机跳变；
4. receiver 复杂度，即 full covariance 是否能预计算和存储。
...
实验15使用的 `tools/run_bler_precoder_alg1.py` 在上述链路基础上增加了一个专用 batch decode 路径：同一个 Monte Carlo trial 中，CDD 和非 CDD precoder 使用相同 TB layout、payload、信道和噪声；脚本先得到不同 precoder 的 coded-bit LLR，再按 CB index 把多个 precoder 的 LLR 堆成 batch，一次送入对应的 Sionna LDPC5GDecoder。这样不改变 BLER 定义和算法比较口径，但能减少重复 decoder 调用。该优化目前只用于算法一 precoder BLER 对比脚本，尚未推广为 `run.py` 主流程的通用 batch-first engine。
...
算法一 CDD vs 非 CDD precoder 的 bit-level BLER 对比：
...
### 21.12 CE-diversity joint precoder design

实验15和实验16说明，precoder 设计不能只优化 Algorithm 1 的 CE NMSE。一个 precoder 可能让等效信道 $g[k]$ 更平滑、更容易估计，但同时降低 ideal-CSI 下的频域分集，使 LDPC codeword 看到的低可靠性 RE 更集中。因此后续 precoder 搜索应同时输出两类指标。
...
对 Monte Carlo trials 取 $\Delta I_{5\%}$，用于近似 ideal-CSI 下最差 5% codeword 的可靠性。若某 precoder 的 CE NMSE 更低但 $\Delta I_{5\%}$ 低于 CDD，说明它可能牺牲了频域分集，BLER gain 不一定成立。
...
候选 precoder 不限于 QPSK。当前第一轮已加入：

- RB-level 4PSK/8PSK/16PSK phase cycling；
- 小 CDD delay + RB-level MPSK hybrid；
- Tx power split + phase cycling；
- smooth sinusoidal phase；
- quadratic/chirp phase；
- smooth random anchor phase。

当前实验16的初步结论是：`RB12 QPSK cycle` 的 CE NMSE gain 大，但 capped-MI 低分位和 $\gamma[k]$ effective rank 明显低于 CDD；`RB1 16PSK` 和 `RB2 8PSK` 的 CE gain 略小，但更能保留频域分集，并在 100 trials/SNR 的 LDPC 验证中改善了 waterfall 区间 BLER。后续正式 BLER 应对这些候选做更密 SNR sweep 和更高 trials。
...
6. 将实验15脚本中的同 TB 多 precoder batch LDPC decode 推广为主流程通用 batch-first 编解码，以提升大规模 sweep 速度。
