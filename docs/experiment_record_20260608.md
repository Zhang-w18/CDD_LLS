...
| CDD delay | 512 samples |
| DMRS spacing | 基础 24 sc；新增 `spacing=18` 和 `spacing=6` 覆盖 DS=1/10/100；DS=100 保留 `spacing=12` 补充 |
| SNR | spacing=24：14,16,18,20,22 dB，DS=100 扩展到 24,26,28 dB；spacing=18：12,14,16,18,20,22,24 dB；spacing=12：14:1:20 dB；spacing=6：10,12,14,16,18,20,22 dB |
| MCS | `nr_256qam` index 20，256QAM，R=682/1024 |
| Trials | 基础 200；新增 spacing=18/6 全部 200；DS=100/sp=24 的 22/24/26/28 dB 扩展到 1000 |
| BLER resolution | 200 trials 时 0.005；1000 trials 时 0.001 |
| PDSCH | 48 RB x 10 symbols，576 sc，30 kHz SCS |
| Antennas | 2Tx x 4Rx |
| DMRS symbols | 2 个，symbol index 2/7 |
| Combined pilot positions | spacing=24 时 24；spacing=18 时 32；spacing=12 时 48；spacing=6 时 96 |
| Alg3 pilot positions/port | spacing=24 时 12；spacing=18 时 16；spacing=12 时 24；spacing=6 时 48 |
| TBS / coded bits | spacing=24：30432 / 45696 bits，9 CB；spacing=18：30352 / 45568 bits，8 CB；spacing=12：30176 / 45312 bits，8 CB；spacing=6：29664 / 44544 bits，8 CB |
...
# Add spacing=6 curves for DS=1/10/100 under the same Experiment 11 setup.
/Users/zhangwei/Downloads/lls_platform_sc_mimo/.venv-sionna1/bin/python \
  tools/run_experiment11_alg3_gain_bler.py \
  --delay-spreads-ns 1,10,100 \
  --dmrs-spacing-sc 6 \
  --snrs 10,12,14,16,18,20,22 \
  --mcs-index 20 \
  --trials 200 \
  --progress-every 50 \
  --reuse-csv outputs/experiment11_alg3_gain_bler/experiment11_alg3_gain_bler_20260619_234019/alg1_alg3_high_mcs_bler.csv \
  --out outputs/experiment11_alg3_gain_bler \
  --fig-dir docs/figures

# Add spacing=18 curves for DS=1/10/100 under the same Experiment 11 setup.
/Users/zhangwei/Downloads/lls_platform_sc_mimo/.venv-sionna1/bin/python \
  tools/run_experiment11_alg3_gain_bler.py \
  --delay-spreads-ns 1,10,100 \
  --dmrs-spacing-sc 18 \
  --snrs 12,14,16,18,20,22,24 \
  --mcs-index 20 \
  --trials 200 \
  --progress-every 50 \
  --reuse-csv outputs/experiment11_alg3_gain_bler/experiment11_alg3_gain_bler_20260630_204310/alg1_alg3_high_mcs_bler.csv \
  --out outputs/experiment11_alg3_gain_bler \
  --fig-dir docs/figures
```

```text
outputs/experiment11_alg3_gain_bler/experiment11_alg3_gain_bler_20260630_224938/
```
...
所有条件画在同一坐标轴中：线型区分算法，Alg1 为虚线、Alg3 为实线；颜色区分 DMRS spacing，`spacing=6/12/18/24` 分别对应蓝/橙/红/绿；marker 区分 DS，`DS=1/10/100 ns` 分别对应圆点/方块/三角。spacing=12 只补充 DS=100 ns；spacing=18 和 spacing=6 均覆盖 DS=1/10/100 的两种算法。
...
spacing=6 新增原始 TB BLER：

| DS | Algorithm | 10 dB | 12 dB | 14 dB | 16 dB | 18 dB | 20 dB | 22 dB |
|---:|---|---:|---:|---:|---:|---:|---:|---:|
| 1 | Alg1 | 1.000 | 1.000 | 0.855 | 0.360 | 0.070 | 0.000 | 0.005 |
| 1 | Alg3 | 1.000 | 1.000 | 0.855 | 0.375 | 0.075 | 0.000 | 0.005 |
| 10 | Alg1 | 1.000 | 0.990 | 0.880 | 0.425 | 0.075 | 0.010 | 0.000 |
| 10 | Alg3 | 1.000 | 0.995 | 0.865 | 0.390 | 0.080 | 0.010 | 0.000 |
| 100 | Alg1 | 1.000 | 1.000 | 0.995 | 0.645 | 0.030 | 0.000 | 0.000 |
| 100 | Alg3 | 1.000 | 1.000 | 1.000 | 0.615 | 0.020 | 0.000 | 0.000 |

spacing=18 新增原始 TB BLER：

| DS | Algorithm | 12 dB | 14 dB | 16 dB | 18 dB | 20 dB | 22 dB | 24 dB |
|---:|---|---:|---:|---:|---:|---:|---:|---:|
| 1 | Alg1 | 0.990 | 0.830 | 0.420 | 0.125 | 0.000 | 0.000 | 0.000 |
| 1 | Alg3 | 0.980 | 0.825 | 0.410 | 0.120 | 0.000 | 0.000 | 0.000 |
| 10 | Alg1 | 1.000 | 0.905 | 0.420 | 0.075 | 0.005 | 0.005 | 0.000 |
| 10 | Alg3 | 1.000 | 0.910 | 0.415 | 0.075 | 0.005 | 0.005 | 0.000 |
| 100 | Alg1 | 1.000 | 1.000 | 1.000 | 0.925 | 0.770 | 0.615 | 0.635 |
| 100 | Alg3 | 1.000 | 1.000 | 0.950 | 0.245 | 0.015 | 0.000 | 0.000 |
...
|---:|---:|---|---:|---:|
| 1 | 6 | Alg1 | 17.564 dB | 18.594 dB |
| 1 | 6 | Alg3 | 17.643 dB | 18.609 dB |
| 1 | 18 | Alg1 | 18.063 dB | 18.708 dB |
| 1 | 18 | Alg3 | 18.051 dB | 18.701 dB |
| 1 | 24 | Alg1 | no crossing | no crossing |
| 1 | 24 | Alg3 | 17.458 dB | 20.262 dB |
| 10 | 6 | Alg1 | 17.668 dB | 20.000 dB |
| 10 | 6 | Alg3 | 17.718 dB | 20.000 dB |
| 10 | 18 | Alg1 | 17.666 dB | 19.488 dB |
| 10 | 18 | Alg3 | 17.664 dB | 19.488 dB |
| 10 | 24 | Alg1 | no crossing | no crossing |
| 10 | 24 | Alg3 | 18.202 dB | 19.584 dB |
| 100 | 6 | Alg1 | 17.215 dB | 18.385 dB |
| 100 | 6 | Alg3 | 17.060 dB | 18.262 dB |
| 100 | 18 | Alg1 | no crossing | no crossing |
| 100 | 18 | Alg3 | 18.642 dB | 20.162 dB |
...
按 `(DS, spacing)` 汇总 Alg1/Alg3 的目标 BLER SNR，以及 Alg3 相对 Alg1 的 SNR gain：

$$
G_{\mathrm{Alg3\ vs\ Alg1}} = \mathrm{SNR}_{\mathrm{Alg1}} - \mathrm{SNR}_{\mathrm{Alg3}}.
$$

正值表示 Alg3 达到同一目标 BLER 所需 SNR 更低；若 Alg1 或 Alg3 未穿过目标 BLER，则记为 `n/a`。

| DS | Spacing | Alg1 @ 10% | Alg3 @ 10% | Alg3 gain @ 10% | Alg1 @ 1% | Alg3 @ 1% | Alg3 gain @ 1% |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 6 | 17.564 | 17.643 | -0.078 | 18.594 | 18.609 | -0.015 |
| 1 | 18 | 18.063 | 18.051 | +0.011 | 18.708 | 18.701 | +0.007 |
| 1 | 24 | n/a | 17.458 | n/a | n/a | 20.262 | n/a |
| 10 | 6 | 17.668 | 17.718 | -0.050 | 20.000 | 20.000 | +0.000 |
| 10 | 18 | 17.666 | 17.664 | +0.002 | 19.488 | 19.488 | +0.000 |
| 10 | 24 | n/a | 18.202 | n/a | n/a | 19.584 | n/a |
| 100 | 6 | 17.215 | 17.060 | +0.155 | 18.385 | 18.262 | +0.124 |
| 100 | 12 | 17.665 | 17.550 | +0.115 | 19.000 | 19.000 | +0.000 |
| 100 | 18 | n/a | 18.642 | n/a | n/a | 20.162 | n/a |
| 100 | 24 | n/a | 21.259 | n/a | n/a | n/a | n/a |
...
补入 spacing=6 与 spacing=18 后，图的结论更完整：

1. **更密 DMRS 主要是在“救回 Alg1”而不是放大 Alg3 相对优势。** `spacing=24` 时，DS=1/10/100 的 Alg1 都没有穿过 10% BLER；改成 `spacing=6` 后，Alg1 在三个 DS 上都进入正常 waterfall，并且 10% BLER 交点分别落在 `17.56 / 17.67 / 17.22 dB`。
2. **spacing=6 下 Alg1 与 Alg3 几乎重合。** 在 200-trial 分辨率下，DS=1 和 DS=10 的 10%/1% BLER 插值差只有 `0.05~0.08 dB`；DS=100 时 Alg3 略好，但 10%/1% 也只领先约 `0.15 / 0.12 dB`。
3. **spacing=18 呈现出中间区域的过渡行为。** DS=1 和 DS=10 时，`spacing=18` 下 Alg1/Alg3 仍几乎重合，10%/1% BLER gain 基本为 0 dB；但 DS=100 时 Alg1 到 24 dB 仍未穿过 10% BLER，而 Alg3 已达到 `18.642 dB @ 10%`、`20.162 dB @ 1%`，说明中等稀疏导频下 Alg3 的优势开始显现。
4. **spacing=6 的绝对 BLER 明显优于 spacing=24。** 例如 DS=100 时，Alg3 从 `spacing=24` 的 `21.259 dB @ 10% BLER` 改善到 `17.060 dB`；Alg1 更是从 `spacing=24` 的 “no crossing through 28 dB” 改善到 `17.215 dB`。
5. **但跨 spacing 不能直接解释为纯算法胜负。** 更密的 `spacing=6/12/18` 都会提高 DMRS overhead、降低 TBS；这张图回答的是 “更密导频后 BLER 怎么变”，不是吞吐最优性；吞吐仍需结合 overhead 单独评估。
...
#### 11.8.2 为什么 `spacing=18` 时只有 `DS=100 ns` 出现明显 Alg3 增益

按照 [`QC_explicit_CDD_link_level_simulation_plan.md`](/Users/zhangwei/Downloads/CDD_LLS/QC_explicit_CDD_link_level_simulation_plan.md)，这里的两个算法含义是：

- `Alg1`：对 **CDD-combined equivalent channel** 直接做 wideband RMMSE；
- `Alg3`：DMRS 不做 CDD，先对每个 port 的 **底层 physical channel** 做 per-port RMMSE，再按已知 `d=512` 合成 equivalent channel。

因此两者的根本差别不是后处理公式，而是“估计对象”不同：`Alg1` 直接估计快速起伏的 `g[k]`，`Alg3` 估计更平滑的 `H_m[k]`。

`d=512` 时，CDD 相位周期是 `4096/512=8` 个子载波。`spacing=24` 是 8 的整数倍，所以会出现很强的 pilot aliasing，三种 `DS` 下 Alg1 都几乎失效。`spacing=18` 以后，这个**整数倍 aliasing 被打破**，所以 `DS=1/10 ns` 下 Alg1 被“救回来”了，Alg3 自然就不再有明显 BLER 优势。

`DS=1/10 ns` 的数据也支持这一点：`spacing=18` 时两种算法的 `ce_nmse_eff` 几乎重合。

| DS | SNR | Alg1 NMSE | Alg3 NMSE | Alg3 gain |
|---:|---:|---:|---:|---:|
| 1 | 18 dB | 6.11e-4 | 6.20e-4 | -0.07 dB |
| 1 | 24 dB | 1.68e-4 | 1.57e-4 | +0.31 dB |
| 10 | 18 dB | 1.75e-3 | 1.75e-3 | +0.00 dB |
| 10 | 24 dB | 5.26e-4 | 5.06e-4 | +0.17 dB |

这说明在 `DS=1/10 ns` 时，虽然 `d=512` 很大，但一旦避免了 `spacing=24` 的 aliasing，matched Alg1 已经能把等效信道估得足够准；256QAM BLER 主要不再受 Alg1/Alg3 的 CE 差异支配。

只有 `DS=100 ns` 仍然保留明显增益，原因是它落在一个“中间区间”：

1. 底层 physical channel 已经不再像 `DS=1/10 ns` 那样近似全带平坦。前面诊断里 `DS=100 ns` 的 branch `B_c,H(0.5)` 约为 `93 sc`，明显小于 `DS=10 ns` 的“`> allocation`”。
2. 但 `Alg3` 估计的仍然是底层 port channel。`spacing=18` 时虽然每个 port 只有 `16` 个 pilot，等效每 port 采样仍比 `93 sc` 的底层相干带宽细，因此 Alg3 还能够持续改善。
3. `Alg1` 估计的是 CDD 后的 equivalent channel。`d=512` 时其等效相干带宽仍极小，`spacing=18` 虽不再是整数倍 aliasing，但对 `g[k]` 依旧明显欠采样，所以 Alg1 进入了“没完全崩掉，但误差降不下去”的状态。

`DS=100 ns, spacing=18` 的 `ce_nmse_eff` 正好体现了这种分离：

| SNR | Alg1 NMSE | Alg3 NMSE | Alg3 gain |
|---:|---:|---:|---:|
| 18 dB | 2.24e-2 | 8.17e-3 | +4.38 dB |
| 20 dB | 2.14e-2 | 6.03e-3 | +5.50 dB |
| 22 dB | 1.98e-2 | 4.31e-3 | +6.62 dB |
| 24 dB | 1.86e-2 | 3.22e-3 | +7.61 dB |

这里最关键的现象不是 “Alg3 特别完美”，而是：

- `Alg1` 的估计误差在高 SNR 仍停留在 `~2e-2` 量级，说明此时主瓶颈已经不是 thermal noise，而是对快速 CDD-equivalent channel 的频域欠采样；
- `Alg3` 的估计误差虽然也不小，但会随 SNR 继续从 `2.42e-2` 降到 `3.22e-3`，因此能把 BLER 从 `18 dB` 的 `0.245` 继续压到 `20 dB` 的 `0.015`，并在 `22/24 dB` 达到 0 error/200 trials。

所以，`spacing=18` 时只有 `DS=100 ns` 出现明显 Alg3 增益，不是因为 `DS=100` 的 Alg3 进入了无误差区，而是因为它恰好满足：

1. `spacing=24` 的整数倍 aliasing 已经解除，Alg1 不再像之前那样完全失效；
2. 但 `DS=100 + d=512` 下的 equivalent channel 对 Alg1 来说仍然太快，残余 CE error 依然很大；
3. 同时底层 branch channel 对 Alg3 来说还没有快到无法用 16 pilot/port 估计。

换句话说，`spacing=18` 把系统推到了一个最能看出两种“估计对象”差异的区间：`Alg1` 估 `g[k]` 仍然偏难，`Alg3` 估 `H_m[k]` 仍然可行，所以只有 `DS=100 ns` 的 BLER 增益被明显放大出来。
